...
df_sender=df_raw_explode.groupby(['sender','year'],as_index=True).agg({"time": "count"}).sort_values("time",ascending=False).reset_index().rename(columns={'time': 'sender_count'})
df_recipient=df_raw_explode.groupby(['recipient','year'],as_index=True).agg({"time": "count"}).sort_values("time",ascending=False).reset_index().rename(columns={'time': 'recipient_count'})

###############################################################################################################
# Merge - outer join both sender and recipient dataframes and fill the null values to zero.
###############################################################################################################

df_merged_sender_recipient=pd.merge(df_sender,df_recipient, how='outer',left_on=('year','sender'),right_on=('year','recipient'))
...
df_merged_sender_recipient=df_merged_sender_recipient[['year','people','sender_count','recipient_count']].sort_values('sender_count', ascending=False).fillna(0)

###############################################################################################################
# Add sender and recipient rank in descending counts. The highest count gets rank in that particular year
###############################################################################################################

df_merged_sender_recipient['sender_count_rank'] = df_merged_sender_recipient.groupby(['year'])['sender_count'].rank("dense",ascending=False)
df_merged_sender_recipient['recipient_count_rank'] = df_merged_sender_recipient.groupby(['year'])['recipient_count'].rank("dense",ascending=False)
df_merge_out=df_merged_sender_recipient.groupby(['people'],as_index=True).agg({"sender_count": "sum","recipient_count": "sum"}).sort_values("sender_count",ascending=False).reset_index()
...
###############################################################################################################
Filtered_records=df_merged_sender_recipient[((df_merged_sender_recipient['sender_count_rank'] >= 1) & (df_merged_sender_recipient['sender_count_rank'] <= 5)) | ((df_merged_sender_recipient['recipient_count_rank'] >= 1) & (df_merged_sender_recipient['recipient_count_rank'] <= 5)) ]
